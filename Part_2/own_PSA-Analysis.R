...
load("Data/our_PSA_results.RData")

# Calculate ICER. Mean difference in costs / mean difference in effects

mean_effects_bsc <- mean(results[3,])
mean_effects_exp <- mean(results[4,])
mean_costs_bsc <- mean(results[1,])
mean_costs_exp <- mean(results[2,])
